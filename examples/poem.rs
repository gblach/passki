...
//!
//! Passkey registration and login with Passki on the Poem web framework, plus optional
//! PRF key derivation.
//!
//! ## Registration
...
struct AuthStartRequest {
    ...
    /// Base64url-encoded PRF input (e.g. "notes-encryption-v1"). When present, the server asks
...
async fn register_start(
    ...
    // An eval-less PRF input just asks whether PRF is supported. Requesting a derivation here
    // instead would report a false negative on authenticators that only support
    ...
    store
        ...
        .lock()
        .unwrap()
        .insert(challenge.challenge.clone(), state);
...
async fn register_finish(
    ...
    let signals = Signals {
        all_accepted_credentials: Some(
            passki.signal_all_accepted_credentials(user.id.as_bytes(), &user.passkeys),
        ),
        current_user_details: Some(passki.signal_current_user_details(
            user.id.as_bytes(),
            &user.username,
            &user.display_name,
        )),
...
async fn auth_finish(
    ...
    let Some((username, passkey)) = found else {
        return Ok(Json(ApiResponse {
            ...
            signals: Some(Signals {
                unknown_credential: Some(passki.signal_unknown_credential(&credential_id)),
                ..Default::default()
            }),
            ..Default::default()
        }));
