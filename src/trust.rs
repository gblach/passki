...
//!
//! Decides whether the certificate chain of an attestation statement leads back to a root
//! the relying party installed with [`crate::Passki::with_attestation_trust`]. Without that link,
//! a statement proves only that it is internally consistent, and a client can mint
//! its own certificate claiming any hardware model it likes.
//!
//! Deliberately a small subset of RFC 5280 path validation: name chaining, signature links,
//! validity periods and the CA constraints. Name constraints, policy mapping and revocation
//! are left out, since attestation chains are short, vendor-controlled, and have no revocation
//! service to consult.
...
/// Maps an X.509 signature algorithm OID to the equivalent COSE identifier, so certificate
/// signatures can reuse the attestation statement verifier.
///
/// RSASSA-PSS is deliberately absent: its hash and salt length live in the algorithm parameters,
/// which a plain identifier cannot carry.
...
/// Verifies that `cert`, whose original encoding is `der`, names `issuer` and carries a signature
/// made with the issuer's key.
...
///
/// `intermediates_below` is how many CA certificates sit between this one and the leaf, which
/// is what a certificate's `pathLenConstraint` caps.
fn check_can_issue(cert: &Certificate, intermediates_below: usize) -> Result<()> {
    ...
    // Key usage is optional, but a CA that states one must have listed certificate signing among
    // the uses it allows.
...
///
/// `chain` is DER-encoded and ordered leaf first, as `x5c` requires. Some authenticators append
/// the root and some do not; either way it counts only because it matches an installed one.
pub(crate) fn validate_trust_path(chain: &[Vec<u8>], anchors: &[Certificate]) -> Result<()> {
    ...
    // An appended root carries no more authority than our own copy of it, so drop
    // it and let the search below decide.
    ...
    // Walk upwards: the certificate at i + 1 issued the one at i, and has i intermediates below
    // it, indices 1 through i, since index 0 is the leaf.
    ...
    // Name and signature pick out which root was meant. Whether that root was allowed to issue
    // the chain is asked separately below, so a misconfigured root does not report as "no root
    // matched".
    let anchor = anchors
        .iter()
    ...
    // A root is trusted by configuration, so its validity period and self-signature
    // are not re-checked, only its authority to issue.
